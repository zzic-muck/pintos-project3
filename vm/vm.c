...
vm_get_victim (void) {
	...
	bool accessed = pml4_is_accessed(thread_current()->pml4, frame->page);

	// 2. If accessed, clear the accessed bit.
	while (accessed) {
		pml4_set_accessed(current_frame->pml4, current_frame->vpage, false);
		// Move to the next frame (you may need to maintain a circular queue or list).
		current_frame = get_next_frame(); // Implement this function to get the next frame.
